...
Pass::Status AzdFixCooperativeMatrixUsePass::Process() {
  ...
  for (const auto& id_and_stat : value_use_stats_) {
    ...
    modified |= RewriteValueType(value_id, new_type_id);
  }
  ...
  switch (value_inst->opcode()) {
    case spv::Op::OpLoad:
      modified |= RewritePointerPointeeType(
          value_inst->GetSingleWordInOperand(0), new_type_id);
      break;
    case spv::Op::OpCopyObject:
      modified |= RewriteValueType(value_inst->GetSingleWordInOperand(0),
                                   new_type_id);
      break;
    case spv::Op::OpPhi:
      for (uint32_t i = 0; i < value_inst->NumInOperands(); i += 2) {
        modified |= RewriteValueType(value_inst->GetSingleWordInOperand(i),
                                     new_type_id);
      }
      break;
    case spv::Op::OpSelect:
      if (value_inst->NumInOperands() > 2) {
        modified |= RewriteValueType(value_inst->GetSingleWordInOperand(1),
                                     new_type_id);
        modified |= RewriteValueType(value_inst->GetSingleWordInOperand(2),
                                     new_type_id);
      }
      break;
    default:
    ...
    switch (user->opcode()) {
      case spv::Op::OpStore:
        if (user->NumInOperands() > 1 &&
          ...
          modified |= RewritePointerPointeeType(
              user->GetSingleWordInOperand(0), new_type_id);
        }
        break;
      case spv::Op::OpCopyObject:
        if (user->NumInOperands() > 0 &&
            user->GetSingleWordInOperand(0) == value_id) {
          modified |= RewriteValueType(user->result_id(), new_type_id);
        }
        break;
      case spv::Op::OpPhi:
        for (uint32_t i = 0; i < user->NumInOperands(); i += 2) {
          if (user->GetSingleWordInOperand(i) == value_id) {
            modified |= RewriteValueType(user->result_id(), new_type_id);
            break;
          }
        }
      ...
      case spv::Op::OpSelect:
        if (user->NumInOperands() > 2 &&
            (user->GetSingleWordInOperand(1) == value_id ||
             user->GetSingleWordInOperand(2) == value_id)) {
          modified |= RewriteValueType(user->result_id(), new_type_id);
        }
        break;
      default:
  ...
  for (const auto& use : uses) {
    ...
    switch (user->opcode()) {
      ...
      case spv::Op::OpStore:
        if (user->NumInOperands() > 1 &&
            user->GetSingleWordInOperand(0) == pointer_id) {
          modified |= RewriteValueType(user->GetSingleWordInOperand(1),
                                       pointee_type_id);
        }
        break;
      case spv::Op::OpCopyObject:
