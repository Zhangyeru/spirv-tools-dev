...
bool HasHalfVec2WithZeroTail(const std::string& disassembly,
  ...
  while (line_begin < disassembly.size()) {
    ...
    const std::string line = disassembly.substr(
        line_begin, line_end == std::string::npos ? std::string::npos
                                                  : line_end - line_begin);
    if (line.find("OpCompositeConstruct %v2half") != std::string::npos &&
...
void ExpectPackedVec2VectorMatmulWithTails(const std::string& disassembly) {
...
std::string MakeFusedConstantBiasStoreModule(uint32_t width) {
...
%shape = OpConstantComposite %v2uint %uint_n %uint_n
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%vec = OpTypeCooperativeVectorHW %float %uint_n
%mat = OpTypeCooperativeMatrixHW %float %uint_n %uint_n
%bias = OpConstantComposite %vec)";
  for (uint32_t i = 0; i < width; ++i) text << " %float_0";
  text << R"(
...
%x = OpCooperativeVectorLoadHW %vec %xbase %int_0
%w = OpCooperativeMatrixLoadHW %mat %wbase %shape %offset %int_0
...
void ExpectDirectMatrixMatmulUnrolled(const std::string& disassembly,
...
OpFunctionEnd
  ...
  ExpectDirectMatrixMatmulUnrolled(disassembly, "%float");
}
...
OpFunctionEnd
  ...
  ExpectPackedVec2MatmulPattern(std::get<0>(result), "%float");
}

TEST_F(HwLowerToStandardTest, LowersChainedPackedVec2MatmulsDirectly) {
  const std::string text = R"(
; CHECK-NOT: HW
...
OpFunctionEnd
  ...
  ExpectPackedVec2MatmulPattern(output, "%float");
  EXPECT_EQ(2u, CountSubstring(output, "OpFunctionCall"));
  EXPECT_GT(CountSubstring(output, "OpExtInst %v2float"), 0u);
...
; CHECK-NOT: OpCooperativeMatrix
; CHECK: OpTypeVector %float 2
; CHECK: OpTypeArray %v2float %uint_6
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndMatch<HwLowerToStandardPass>(text, true);
  ExpectDirectMatrixMatmulUnrolled(std::get<0>(result), "%float");
}
...
; CHECK-NOT: OpCooperativeMatrix
; CHECK: OpTypeVector %float 2
; CHECK: OpTypeArray %v2float %uint_4
; CHECK: OpTypeArray %v2float %uint_8
; CHECK: OpExtInst %v2float
; CHECK-SAME: Fma
OpCapability Shader
...
; CHECK: OpTypeArray %float %uint_15
; CHECK: OpTypeVector %float 2
; CHECK: OpTypeArray %v2float %uint_20
; CHECK: OpTypeArray %v2float %uint_12
; CHECK: OpFunctionCall
; CHECK: OpExtInst %v2float
; CHECK-SAME: Fma
; CHECK: OpReturnValue
...
OpFunctionEnd
  ...
  EXPECT_NE(std::string::npos, output.find("OpTypeVector %float 2"));
  EXPECT_NE(std::string::npos, output.find("OpTypeArray %v2float %uint_20"));
  EXPECT_NE(std::string::npos, output.find("OpTypeArray %v2float %uint_12"));
  EXPECT_GT(CountSubstring(output, "OpExtInst %v2float"), 0u);
  EXPECT_GT(CountSubstring(output, " Fma "), 0u);
  EXPECT_EQ(0u, CountSubstring(output, "OpLoopMerge"));
  EXPECT_GT(CountSubstring(output, "OpFunctionCall"), 0u);
  EXPECT_GT(CountSubstring(output, "OpReturnValue"), 0u);
  EXPECT_EQ(0u, CountSubstring(output, "OpVectorTimesScalar"));
}

TEST_F(HwLowerToStandardTest, LowersVectorMatrixMul8x8F32Tile2) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK-NOT: CooperativeMatrixKHR
; CHECK-NOT: OpTypeCooperativeMatrixKHR
...
; CHECK: OpTypeVector %float 2
; CHECK: OpTypeArray %v2float %uint_4
; CHECK: OpTypeArray %v2float %uint_32
; CHECK: OpExtInst %v2float
; CHECK-SAME: Fma
OpCapability Shader
OpCapability CooperativeVectorHW
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndMatch<HwLowerToStandardPass>(text, true);
  ExpectPackedVec2Math(std::get<0>(result), "%float");
}

TEST_F(HwLowerToStandardTest, LowersVectorMatrixMulAdd8x8F32Tile2WithBias) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK-NOT: CooperativeMatrixKHR
; CHECK-NOT: OpTypeCooperativeMatrixKHR
...
TEST_F(HwLowerToStandardTest, LowersEvenVectorMatrixMulNWithVec2Packing) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK-NOT: CooperativeMatrixKHR
; CHECK-NOT: OpTypeCooperativeMatrixKHR
...
; CHECK: OpTypeVector %float 2
; CHECK: OpTypeArray %v2float %uint_4
; CHECK: OpTypeArray %v2float %uint_5
; CHECK: OpTypeArray %v2float %uint_40
; CHECK: OpExtInst %v2float
; CHECK-SAME: Fma
OpCapability Shader
OpCapability CooperativeVectorHW
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndMatch<HwLowerToStandardPass>(text, true);
  const std::string& output = std::get<0>(result);
  ExpectPackedVec2Math(output, "%float");
  EXPECT_NE(std::string::npos, output.find("OpTypeVector %float 2"));
  EXPECT_GT(CountSubstring(output, "OpTypeArray %v2"), 0u);
}
...
TEST_F(HwLowerToStandardTest,
  ...
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK: OpFunctionCall %_arr_v2float_uint_4
...
OpFunctionEnd
  ...
  EXPECT_NE(std::string::npos,
            disassembly.find("OpFunctionCall %_arr_v2float_uint_4"));
...
TEST_F(HwLowerToStandardTest, DISABLED_FusedVectorMatmulStoreStreamsToSsbo) {
  const std::string text = R"(
; CHECK-NOT: HW
OpCapability Shader
OpCapability CooperativeVectorHW
...
OpFunctionEnd
  ...
  EXPECT_EQ(0u,
            CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_4"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpDot %float"));
  EXPECT_EQ(2u, CountSubstring(disassembly, " Fma "));
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpFAdd %float"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpUDiv"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpUMod"));
}

TEST_F(HwLowerToStandardTest,
       DISABLED_FusedVectorMatmulAddWithConstantBiasStoreStreamsToSsbo) {
  const std::string text = R"(
; CHECK-NOT: HW
OpCapability Shader
OpCapability CooperativeVectorHW
...
OpFunctionEnd
  ...
  EXPECT_EQ(0u,
            CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_4"));
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpDot %float"));
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpFAdd %v2float"));
}

TEST_F(HwLowerToStandardTest,
       DISABLED_FusedConstantBiasStoreSupportsEightVec2Packs) {
  const auto fused = SinglePassRunAndDisassemble<HwLowerToStandardPass>(
      MakeFusedConstantBiasStoreModule(16), true, true);
  const std::string& fused_disassembly = std::get<0>(fused);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(fused));
  ExpectNoHwOrCoopMatrix(fused_disassembly);
  EXPECT_EQ(2u, CountSubstring(fused_disassembly, "OpDot %float"));
  EXPECT_EQ(0u, CountSubstring(fused_disassembly,
                               "OpTypeFunction %_arr_v2float_uint_8"));

  const auto fallback = SinglePassRunAndDisassemble<HwLowerToStandardPass>(
      MakeFusedConstantBiasStoreModule(18), true, true);
  const std::string& fallback_disassembly = std::get<0>(fallback);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(fallback));
  ExpectNoHwOrCoopMatrix(fallback_disassembly);
  EXPECT_EQ(0u, CountSubstring(fallback_disassembly, "OpDot %float"));
  EXPECT_GT(CountSubstring(fallback_disassembly,
                           "OpTypeFunction %_arr_v2float_uint_9"),
            0u);
}
...
OpFunctionEnd
  ...
  const std::string& disassembly = std::get<0>(result);
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_NE(std::string::npos, disassembly.find("OpBitcast %v2float"));
}
...
OpFunctionEnd
  ...
  const std::string& disassembly = std::get<0>(result);
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_4"),
            0u);
}
...
OpFunctionEnd
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpFunctionCall %_arr_v2float_uint_32"),
            0u);
  ...
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoad %_arr_v2half_uint_320"));
  EXPECT_EQ(0u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2half_uint_320"));
}
...
OpFunctionEnd
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpFunctionCall %_arr_v2float_uint_8"),
            0u);
  EXPECT_GT(CountSubstring(disassembly, "OpFConvert %v2float"), 0u);
  EXPECT_GT(CountSubstring(disassembly, "OpExtInst %v2float"), 0u);
  EXPECT_GT(CountSubstring(disassembly, " Fma "), 0u) << disassembly;
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpLoopMerge")) << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpUDiv %uint"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpUMod %uint"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoad %_arr_v2half_uint_512"));
  EXPECT_EQ(0u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2half_uint_512"));
}
...
OpFunctionEnd
  ...
  for (const VolatileCase& test_case : cases) {
    ...
    EXPECT_GT(CountSubstring(disassembly,
                             "OpFunctionParameter %_arr_v2float_uint_32"),
              0u);
  }
}
...
TEST_F(HwLowerToStandardTest,
       DirectMixedPrecisionVectorMatrixMulAddStreamsConvertedBiasWithTails) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability FloatControls2
...
OpEntryPoint GLCompute %main "main"
OpExecutionMode %main LocalSize 1 1 1
OpDecorate %bias_converted FPFastMathMode Fast
OpDecorate %_runtimearr_half ArrayStride 2
OpMemberDecorate %HalfBuf 0 Offset 0
OpDecorate %HalfBuf Block
%uint = OpTypeInt 32 0
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape = OpConstantComposite %v2uint %uint_10 %uint_3
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%input_type = OpTypeCooperativeVectorHW %half %uint_10
%bias_source_type = OpTypeCooperativeVectorHW %half %uint_3
%result_type = OpTypeCooperativeVectorHW %float %uint_3
%matrix_type = OpTypeCooperativeMatrixHW %half %uint_10 %uint_3
%_runtimearr_half = OpTypeRuntimeArray %half
%HalfBuf = OpTypeStruct %_runtimearr_half
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%input_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%matrix_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%bias_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
%_ptr_Function_bias_source_type = OpTypePointer Function %bias_source_type
%_ptr_Function_result_type = OpTypePointer Function %result_type
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
%entry = OpLabel
%bias_temp = OpVariable %_ptr_Function_bias_source_type Function
%bias_copy = OpVariable %_ptr_Function_bias_source_type Function
%bias_result = OpVariable %_ptr_Function_result_type Function
%input_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %input_buf %int_0
%matrix_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %matrix_buf %int_0
%bias_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bias_buf %int_0
%input = OpCooperativeVectorLoadHW %input_type %input_base %uint_0
%matrix = OpCooperativeMatrixLoadHW %matrix_type %matrix_base %shape %offset %int_0
%bias_loaded = OpCooperativeVectorLoadHW %bias_source_type %bias_base %uint_0
OpStore %bias_temp %bias_loaded
%bias_temp_value = OpLoad %bias_source_type %bias_temp
OpStore %bias_copy %bias_temp_value
%bias_f16 = OpLoad %bias_source_type %bias_copy
%bias_converted = OpFConvert %result_type %bias_f16
OpStore %bias_result %bias_converted
%bias = OpLoad %result_type %bias_result
%result = OpCooperativeVectorMatrixMulAddHW %result_type %input %matrix %bias
OpReturn
OpFunctionEnd
)";
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(
      0u, CountSubstring(disassembly, "OpFunctionParameter %_arr_float_uint_3"))
      << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoad %_arr_half_uint_3"))
      << disassembly;
  EXPECT_GT(CountSubstring(disassembly, "OpFConvert %float"), 0u);
  EXPECT_GT(CountSubstring(disassembly, "OpFConvert %v2float"), 0u);
  EXPECT_GE(CountSubstring(disassembly, "FPFastMathMode Fast"), 2u);
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoopMerge")) << disassembly;
  EXPECT_FALSE(HasHalfVec2WithZeroTail(disassembly, 1u)) << disassembly;
}

TEST_F(HwLowerToStandardTest,
       DirectSamePrecisionVectorMatrixMulAdd5x3HandlesBothTails) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeVectorHW
OpCapability CooperativeMatrixHW
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape = OpConstantComposite %v2uint %uint_5 %uint_3
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%input_type = OpTypeCooperativeVectorHW %float %uint_5
%result_type = OpTypeCooperativeVectorHW %float %uint_3
%matrix_type = OpTypeCooperativeMatrixHW %float %uint_5 %uint_3
%_runtimearr_float = OpTypeRuntimeArray %float
%Buf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_Buf = OpTypePointer StorageBuffer %Buf
%input_buf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
...
OpFunctionEnd
  ...
  EXPECT_EQ(
      0u, CountSubstring(disassembly, "OpFunctionParameter %_arr_float_uint_3"))
      << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFConvert")) << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoopMerge")) << disassembly;
  EXPECT_GT(CountSubstring(disassembly, " Fma "), 0u) << disassembly;
}

TEST_F(HwLowerToStandardTest, ConvertedBiasWithNonzeroOffsetStaysDirect) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%source_type = OpTypeCooperativeVectorHW %half %uint_4
%result_type = OpTypeCooperativeVectorHW %float %uint_4
%matrix_type = OpTypeCooperativeMatrixHW %half %uint_4 %uint_4
%_runtimearr_half = OpTypeRuntimeArray %half
%HalfBuf = OpTypeStruct %_runtimearr_half
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%input_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
...
%matrix_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %matrix_buf %int_0
%bias_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bias_buf %int_0
%input = OpCooperativeVectorLoadHW %source_type %input_base %uint_0
%matrix = OpCooperativeMatrixLoadHW %matrix_type %matrix_base %shape %offset %int_0
%bias_f16 = OpCooperativeVectorLoadHW %source_type %bias_base %uint_1
%bias = OpFConvert %result_type %bias_f16
%result = OpCooperativeVectorMatrixMulAddHW %result_type %input %matrix %bias
OpReturn
OpFunctionEnd
)";
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(0u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2float_uint_2"))
      << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoad %_arr_v2half_uint_2"))
      << disassembly;
  EXPECT_GT(CountSubstring(disassembly, "OpFConvert %v2float"), 0u)
      << disassembly;
}
...
TEST_F(HwLowerToStandardTest,
       ConvertedBiasAliasedInterveningStoreKeepsBiasAsValue) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
OpMemberDecorate %HalfBuf 0 Offset 0
OpDecorate %HalfBuf Block
OpDecorate %bias_buf Aliased
OpDecorate %other_buf Aliased
%uint = OpTypeInt 32 0
%uint_0 = OpConstant %uint 0
%uint_4 = OpConstant %uint 4
%int = OpTypeInt 32 1
...
%matrix_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%bias_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%other_buf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
...
%matrix_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %matrix_buf %int_0
%bias_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bias_buf %int_0
%other_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %other_buf %int_0
%other_element = OpAccessChain %_ptr_StorageBuffer_half %other_base %uint_0
%input = OpCooperativeVectorLoadHW %source_type %input_base %uint_0
%matrix = OpCooperativeMatrixLoadHW %matrix_type %matrix_base %shape %offset %int_0
%bias_f16 = OpCooperativeVectorLoadHW %source_type %bias_base %uint_0
OpStore %other_element %half_1
%bias = OpFConvert %result_type %bias_f16
%result = OpCooperativeVectorMatrixMulAddHW %result_type %input %matrix %bias
OpReturn
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(
      CountSubstring(disassembly, "OpFunctionParameter %_arr_v2float_uint_2"),
      0u)
      << disassembly;
  EXPECT_GE(CountSubstring(disassembly, " Aliased"), 2u) << disassembly;
}

TEST_F(HwLowerToStandardTest,
       ConvertedBiasVolatileFunctionTransportKeepsFallback) {
  const std::string base_text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
%main = OpFunction %void None %fn
%entry = OpLabel
%bias_transport = OpVariable %_ptr_Function_result_type Function
%input_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %input_buf %int_0
%matrix_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %matrix_buf %int_0
%bias_base = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bias_buf %int_0
%input = OpCooperativeVectorLoadHW %source_type %input_base %uint_0
...
%bias_f16 = OpCooperativeVectorLoadHW %source_type %bias_base %uint_0
%bias_converted = OpFConvert %result_type %bias_f16
OpStore %bias_transport %bias_converted
%bias = OpLoad %result_type %bias_transport__MEMORY_ACCESS__
%result = OpCooperativeVectorMatrixMulAddHW %result_type %input %matrix %bias
OpReturn
OpFunctionEnd
)";
  ...
  struct VolatileCase {
    ...
    const char* decoration;
    const char* memory_access;
    bool expect_bias_value_parameter;
  };
  const VolatileCase cases[] = {
      {"memory operand", "", " Volatile", true},
      {"variable decoration", "OpDecorate %bias_transport Volatile", "", false},
  };
  auto replace_all = [](std::string* text, const std::string& from,
                        const std::string& to) {
    size_t position = 0;
  ...
  for (const VolatileCase& test_case : cases) {
    ...
    EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
    ExpectNoHwOrCoopMatrix(disassembly);
    EXPECT_GT(CountSubstring(disassembly, " Volatile"), 0u) << disassembly;
    if (test_case.expect_bias_value_parameter) {
      EXPECT_GT(CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2float_uint_2"),
                0u)
          << disassembly;
...
TEST_F(HwLowerToStandardTest,
       ConvertedBiasPreservesExplicitFPFastMathModeNone) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability FloatControls2
...
OpDecorate %bias FPFastMathMode None
OpDecorate %_runtimearr_half ArrayStride 2
OpMemberDecorate %HalfBuf 0 Offset 0
OpDecorate %HalfBuf Block
%uint = OpTypeInt 32 0
...
OpFunctionEnd
  ...
  EXPECT_EQ(0u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2float_uint_2"))
      << disassembly;
  EXPECT_GT(CountSubstring(disassembly, "FPFastMathMode None"), 0u)
      << disassembly;
}
...
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(
      CountSubstring(disassembly, "OpFunctionParameter %_arr_v2float_uint_32"),
      0u);
  EXPECT_GT(CountSubstring(disassembly, "OpLoad %_arr_v2float_uint_32"), 0u);
}

TEST_F(HwLowerToStandardTest,
       VectorDirectUsesUnrolledAtBoundaryAndRolledAboveLimit) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
OpFunctionEnd
  ...
  auto unrolled = SinglePassRunAndDisassemble<HwLowerToStandardPass>(
      ...
      1024u, 30ull);
  const std::string& unrolled_disassembly = std::get<0>(unrolled);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(unrolled));
  ExpectNoHwOrCoopMatrix(unrolled_disassembly);
  EXPECT_GT(
      CountSubstring(unrolled_disassembly, "OpFunctionCall %_arr_float_uint_3"),
      0u);
  EXPECT_EQ(0u, CountSubstring(unrolled_disassembly, "OpLoopMerge"));
  EXPECT_GT(CountSubstring(unrolled_disassembly, "OpFConvert %v2float"), 0u);
  EXPECT_GT(CountSubstring(unrolled_disassembly, " Fma "), 0u);

  auto rolled = SinglePassRunAndDisassemble<HwLowerToStandardPass>(
      text, true, true, HwLowerToStandardPass::LoweringMode::kPreferPackedVec2,
      HwLowerToStandardPass::CompletenessMode::kCooperativeOnly, 1024u, 1024ull,
      1024u, 29ull);
  const std::string& rolled_disassembly = std::get<0>(rolled);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(rolled));
  ExpectNoHwOrCoopMatrix(rolled_disassembly);
  EXPECT_GT(
      CountSubstring(rolled_disassembly, "OpFunctionCall %_arr_float_uint_3"),
      0u);
...
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(0u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2float_uint_8"));
...
OpFunctionEnd
  ...
  const std::string& disassembly = std::get<0>(result);
  ExpectNoHwOrCoopMatrix(disassembly);
  ExpectPackedVec2MatmulPattern(disassembly, "%float");
}
...
TEST_F(HwLowerToStandardTest,
       DirectMixedPrecisionMatrixMulAdd2x5x3HandlesTails) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeMatrixHW
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape_a = OpConstantComposite %v2uint %uint_2 %uint_5
%shape_b = OpConstantComposite %v2uint %uint_5 %uint_3
%shape_c = OpConstantComposite %v2uint %uint_2 %uint_3
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%mat_a = OpTypeCooperativeMatrixHW %half %uint_2 %uint_5
%mat_b = OpTypeCooperativeMatrixHW %half %uint_5 %uint_3
%mat_c = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3
%_runtimearr_half = OpTypeRuntimeArray %half
%_runtimearr_float = OpTypeRuntimeArray %float
%HalfBuf = OpTypeStruct %_runtimearr_half
%FloatBuf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%_ptr_StorageBuffer_FloatBuf = OpTypePointer StorageBuffer %FloatBuf
%abuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%bbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%cbuf = OpVariable %_ptr_StorageBuffer_FloatBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
%_ptr_StorageBuffer__runtimearr_float = OpTypePointer StorageBuffer %_runtimearr_float
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
%entry = OpLabel
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%a = OpCooperativeMatrixLoadHW %mat_a %abase %shape_a %offset %int_0
%b = OpCooperativeMatrixLoadHW %mat_b %bbase %shape_b %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat_c %cbase %shape_c %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat_c %a %b %c
OpReturn
OpFunctionEnd
)";
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoopMerge"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFunctionParameter"));
  EXPECT_GT(CountSubstring(disassembly, "OpFConvert %v2float"), 0u);
  EXPECT_GT(CountSubstring(disassembly, "OpExtInst %v2float"), 0u);
}

TEST_F(HwLowerToStandardTest,
       DirectSamePrecisionMatrixMulAdd2x5x3HandlesTails) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%shape_c = OpConstantComposite %v2uint %uint_2 %uint_3
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%mat_a = OpTypeCooperativeMatrixHW %float %uint_2 %uint_5
%mat_b = OpTypeCooperativeMatrixHW %float %uint_5 %uint_3
%mat_c = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3
%_runtimearr_float = OpTypeRuntimeArray %float
%Buf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_Buf = OpTypePointer StorageBuffer %Buf
...
OpFunctionEnd
  ...
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoopMerge"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFunctionParameter"));
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFConvert"));
  EXPECT_GT(CountSubstring(disassembly, "OpExtInst %v2float"), 0u);
}

TEST_F(HwLowerToStandardTest,
       IrregularMatrixDirectVolatileFunctionTransportKeepsFallback) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%main = OpFunction %void None %fn
%entry = OpLabel
%a_transport = OpVariable %_ptr_Function_mat_a Function
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%a_loaded = OpCooperativeMatrixLoadHW %mat_a %abase %shape_a %offset %int_0
OpStore %a_transport %a_loaded
%a = OpLoad %mat_a %a_transport Volatile
%b = OpCooperativeMatrixLoadHW %mat_b %bbase %shape_b %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat_c %cbase %shape_c %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat_c %a %b %c
OpReturn
OpFunctionEnd
  ...
  EXPECT_GT(CountSubstring(disassembly, " Volatile"), 0u) << disassembly;
  EXPECT_GT(
      CountSubstring(disassembly, "OpFunctionParameter %_arr_float_uint_10"),
      0u)
      << disassembly;
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpLoopMerge")) << disassembly;
}
...
TEST_F(HwLowerToStandardTest,
       DirectMixedPrecisionMatrixMulAddDoesNotAbsorbConvertedC) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK: OpFConvert %float
; CHECK: [[CONVERTED_C_ARG:%\w+]] = OpLoad %_arr_float_uint_6
; CHECK: {{%\w+}} = OpFunctionCall %_arr_float_uint_6 [[DIRECT_HELPER:%\w+]] [[CONVERTED_C_ARG]]
...
OpEntryPoint GLCompute %main "main"
OpExecutionMode %main LocalSize 1 1 1
OpDecorate %cf_source FPFastMathMode NotNaN
...
%mat_a = OpTypeCooperativeMatrixHW %half %uint_2 %uint_5
%mat_b = OpTypeCooperativeMatrixHW %half %uint_5 %uint_3
%mat_ch = OpTypeCooperativeMatrixHW %half %uint_2 %uint_3 MatrixUseAHW
%mat_cf_source = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3 MatrixUseAHW
%mat_cf = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3 MatrixAccumulatorHW
%_runtimearr_half = OpTypeRuntimeArray %half
%HalfBuf = OpTypeStruct %_runtimearr_half
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%abuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%bbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%cbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
...
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %cbuf %int_0
%a = OpCooperativeMatrixLoadHW %mat_a %abase %shape_a %offset %int_0
%b = OpCooperativeMatrixLoadHW %mat_b %bbase %shape_b %offset %int_0
%ch = OpCooperativeMatrixLoadHW %mat_ch %cbase %shape_c %offset %int_0
%cf_source = OpFConvert %mat_cf_source %ch
%cf = OpBitcast %mat_cf %cf_source
%d = OpCooperativeMatrixMulAddHW %mat_cf %a %b %cf
OpReturn
OpFunctionEnd
)";
  ...
  const auto expect_result_decoration = [&disassembly](
    ...
    const std::string opcode_marker = " = " + opcode_and_type;
    size_t opcode_pos = 0;
    bool found = false;
    while ((opcode_pos = disassembly.find(opcode_marker, opcode_pos)) !=
           std::string::npos) {
      size_t line_begin = disassembly.rfind('\n', opcode_pos);
      line_begin = line_begin == std::string::npos ? 0 : line_begin + 1;
      const size_t id_begin = disassembly.find_first_not_of(" \t", line_begin);
      if (id_begin != std::string::npos && id_begin < opcode_pos) {
        const std::string result_id =
            disassembly.substr(id_begin, opcode_pos - id_begin);
        if (disassembly.find("OpDecorate " + result_id + " " + decoration) !=
            std::string::npos) {
          found = true;
          break;
        }
      }
      opcode_pos += opcode_marker.size();
    }
...
TEST_F(HwLowerToStandardTest,
       DirectMatrixMulAddComponentBitcastValueFallsBackSafely) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%uint_3 = OpConstant %uint 3
%float = OpTypeFloat 32
%mat_bits = OpTypeCooperativeMatrixHW %uint %uint_2 %uint_3
%mat_a = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3
%mat_b = OpTypeCooperativeMatrixHW %float %uint_3 %uint_2
%mat_c = OpTypeCooperativeMatrixHW %float %uint_2 %uint_2
%bits = OpUndef %mat_bits
%b = OpUndef %mat_b
%c = OpUndef %mat_c
%main = OpFunction %void None %fn
%entry = OpLabel
%a = OpBitcast %mat_a %bits
%d = OpCooperativeMatrixMulAddHW %mat_c %a %b %c
OpReturn
OpFunctionEnd
)";
  ...
  const std::string& disassembly = std::get<0>(result);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectScalarFallbackMath(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpBitcast %float"), 0u);
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFunctionCall"));
}

TEST_F(HwLowerToStandardTest,
       DirectMatrixMulAddComponentBitcastLoadFallsBackSafely) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
OpDecorate %_runtimearr_float ArrayStride 4
OpMemberDecorate %UintBuf 0 Offset 0
OpMemberDecorate %FloatBuf 0 Offset 0
OpDecorate %UintBuf Block
OpDecorate %FloatBuf Block
%void = OpTypeVoid
%fn = OpTypeFunction %void
%uint = OpTypeInt 32 0
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape_a = OpConstantComposite %v2uint %uint_2 %uint_3
%shape_b = OpConstantComposite %v2uint %uint_3 %uint_2
%shape_c = OpConstantComposite %v2uint %uint_2 %uint_2
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%mat_bits = OpTypeCooperativeMatrixHW %uint %uint_2 %uint_3
%mat_a = OpTypeCooperativeMatrixHW %float %uint_2 %uint_3
%mat_b = OpTypeCooperativeMatrixHW %float %uint_3 %uint_2
...
%_runtimearr_float = OpTypeRuntimeArray %float
%UintBuf = OpTypeStruct %_runtimearr_uint
%FloatBuf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_UintBuf = OpTypePointer StorageBuffer %UintBuf
%_ptr_StorageBuffer_FloatBuf = OpTypePointer StorageBuffer %FloatBuf
%_ptr_StorageBuffer__runtimearr_uint = OpTypePointer StorageBuffer %_runtimearr_uint
%_ptr_StorageBuffer__runtimearr_float = OpTypePointer StorageBuffer %_runtimearr_float
%abuf = OpVariable %_ptr_StorageBuffer_UintBuf StorageBuffer
%bbuf = OpVariable %_ptr_StorageBuffer_FloatBuf StorageBuffer
%cbuf = OpVariable %_ptr_StorageBuffer_FloatBuf StorageBuffer
%main = OpFunction %void None %fn
%entry = OpLabel
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_uint %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%bits = OpCooperativeMatrixLoadHW %mat_bits %abase %shape_a %offset %int_0
%a = OpBitcast %mat_a %bits
%b = OpCooperativeMatrixLoadHW %mat_b %bbase %shape_b %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat_c %cbase %shape_c %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat_c %a %b %c
...
OpFunctionEnd
  ...
  ExpectScalarFallbackMath(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpBitcast %float"), 0u);
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpFunctionCall %v2float"));
}

TEST_F(HwLowerToStandardTest,
       DirectMatrixMulAddFallsBackWhenLoadHasUnsafeSharedUser) {
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%c = OpCooperativeMatrixLoadHW %mat_c %cbase %shape_c %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat_c %a %b %c
%a0 = OpCompositeExtract %float %a 0 0
OpReturn
OpFunctionEnd
)";
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectScalarFallbackMath(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpCompositeExtract %float"), 0u);
  EXPECT_EQ(0u, CountSubstring(disassembly, "OpFunctionCall"));
}
...
TEST_F(HwLowerToStandardTest, DISABLED_FusedMatrixMatmulStoreStreamsToSsbo) {
  const std::string text = R"(
; CHECK-NOT: HW
OpCapability Shader
OpCapability CooperativeMatrixHW
...
OpFunctionEnd
  ...
  EXPECT_EQ(
      0u, CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_32"));
}
...
OpFunctionEnd
  ...
  const std::string& disassembly = std::get<0>(result);
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_GT(CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_32"),
            0u);
...
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(1u, CountSubstring(disassembly,
                               "OpFunctionParameter %_arr_v2float_uint_4"));
}
...
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_NE(std::string::npos,
            disassembly.find("OpFunctionParameter %_arr_v2float_uint_4"));
}
...
OpFunctionEnd
  ...
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(
      1u, CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_4\n"));
  EXPECT_EQ(2u,
            CountSubstring(disassembly, "OpFunctionCall %_arr_v2float_uint_4"));
}
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndMatch<HwLowerToStandardPass>(text, true);
  ExpectPackedVec2MatmulPattern(std::get<0>(result), "%half");
}
...
; CHECK: OpTypeFloat 16
; CHECK: OpTypeVector %half 2
; CHECK: OpTypeArray %v2half %uint_32
; CHECK: OpExtInst %v2half
; CHECK-SAME: Fma
OpCapability Shader
OpCapability Float16
...
TEST_F(HwLowerToStandardTest, F32MatrixMulAddUsesDirectPathWhenUnaligned) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK: OpTypeArray %float %uint_15
...
TEST_F(HwLowerToStandardTest, F32VectorMatrixMulUsesPackedTailsWhenUnaligned) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK: OpTypeArray %float %uint_5
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndMatch<HwLowerToStandardPass>(text, true);
  ExpectPackedVec2VectorMatmulWithTails(std::get<0>(result));
}

TEST_F(HwLowerToStandardTest,
       F32VectorMatrixMulAddUsesPackedTailsWhenUnaligned) {
  const std::string text = R"(
; CHECK-NOT: HW
; CHECK: OpTypeArray %float %uint_5
; CHECK: OpTypeArray %float %uint_3
...
OpFunctionEnd
  ...
  EXPECT_GE(CountSubstring(disassembly, "OpTypeFunction %_arr_v2float_uint_8"),
            1u);
...
TEST_F(HwLowerToStandardTest,
       MatrixDirectUsesUnrolledAtBoundaryAndRolledAboveLimit) {
  const std::string text = R"(
; CHECK-NOT: HW
OpCapability Shader
OpCapability Float16
...
%b_type = OpTypeCooperativeMatrixHW %half %uint_4 %uint_4
...
OpFunctionEnd
  ...
  const std::string& disassembly = std::get<0>(result);
  ExpectNoHwOrCoopMatrix(disassembly);
  // The integer lowering retains its scalar N/K loops.  The f32 vector path is
  // compile-time specialized into packed operations, including its short tail.
...
OpFunctionEnd
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_NE(std::string::npos, disassembly.find("OpTypeVector %float 2"));
  EXPECT_NE(std::string::npos,
            disassembly.find("OpTypeArray %v2float %uint_12"));
...
OpFunctionEnd
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_NE(std::string::npos, disassembly.find("OpTypeVector %half 2"));
  EXPECT_NE(std::string::npos, disassembly.find("OpTypeArray %v2half %uint_8"));
  EXPECT_GT(CountSubstring(disassembly, " FMin "), 0u);
  EXPECT_GT(CountSubstring(disassembly, " FMax "), 0u);
}
...
// ──────────────────────────────────────────────────────────────────
// P0: Rolled Direct Path
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest, RolledDirectMatrixMulAddGeneratesStructuredLoop) {
  // M=16, K=16, N=16 → MAC=4096.  Threshold set to 2048 → rolled.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
OpFunctionEnd
  ...
  auto result = SinglePassRunAndDisassemble<HwLowerToStandardPass>(
      text, true, true, HwLowerToStandardPass::LoweringMode::kPreferPackedVec2,
      HwLowerToStandardPass::CompletenessMode::kCooperativeOnly, 1048576,
      16777216, 4096, 2048);
  const std::string& disassembly = std::get<0>(result);
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
...
TEST_F(HwLowerToStandardTest,
       RolledDirectVectorMatrixMulGeneratesStructuredLoop) {
  // K=64, N=128 → MAC=8192 > 4096 (default threshold) → rolled.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeVectorHW
OpCapability CooperativeMatrixHW
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape = OpConstantComposite %v2uint %uint_64 %uint_128
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%vec64 = OpTypeCooperativeVectorHW %float %uint_64
%vec128 = OpTypeCooperativeVectorHW %float %uint_128
%mat64x128 = OpTypeCooperativeMatrixHW %float %uint_64 %uint_128
%_runtimearr_float = OpTypeRuntimeArray %float
%Buf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_Buf = OpTypePointer StorageBuffer %Buf
%xbuf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
...
%xbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %xbuf %int_0
%wbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %wbuf %int_0
%x = OpCooperativeVectorLoadHW %vec64 %xbase %int_0
%w = OpCooperativeMatrixLoadHW %mat64x128 %wbase %shape %offset %int_0
%y = OpCooperativeVectorMatrixMulHW %vec128 %x %w
OpReturn
OpFunctionEnd
)";
...
TEST_F(HwLowerToStandardTest, RolledDirectMixedPrecisionInsertsFConvert) {
  // f16 A(8×16) × f16 B(16×8) + f32 C → f32, MAC=1024, threshold=512 → rolled.
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeMatrixHW
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape_a = OpConstantComposite %v2uint %uint_8 %uint_16
%shape_b = OpConstantComposite %v2uint %uint_16 %uint_8
%shape_c = OpConstantComposite %v2uint %uint_8 %uint_8
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%mat_a = OpTypeCooperativeMatrixHW %half %uint_8 %uint_16
%mat_b = OpTypeCooperativeMatrixHW %half %uint_16 %uint_8
%mat_c = OpTypeCooperativeMatrixHW %float %uint_8 %uint_8
%_runtimearr_half = OpTypeRuntimeArray %half
%_runtimearr_float = OpTypeRuntimeArray %float
%HalfBuf = OpTypeStruct %_runtimearr_half
%FloatBuf = OpTypeStruct %_runtimearr_float
...
TEST_F(HwLowerToStandardTest, MixedOperandMatrixDirectUsesF32RolledLimit) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeMatrixHW
...
%shape = OpConstantComposite %v2uint %uint_16 %uint_16
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%mat_half = OpTypeCooperativeMatrixHW %half %uint_16 %uint_16
%mat_float = OpTypeCooperativeMatrixHW %float %uint_16 %uint_16
%_runtimearr_half = OpTypeRuntimeArray %half
%_runtimearr_float = OpTypeRuntimeArray %float
%HalfBuf = OpTypeStruct %_runtimearr_half
%FloatBuf = OpTypeStruct %_runtimearr_float
...
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%a = OpCooperativeMatrixLoadHW %mat_half %abase %shape %offset %int_0
%b = OpCooperativeMatrixLoadHW %mat_float %bbase %shape %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat_float %cbase %shape %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat_float %a %b %c
OpReturn
OpFunctionEnd
)";
...
TEST_F(HwLowerToStandardTest, MixedOperandVectorDirectUsesF32RolledLimit) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%input_type = OpTypeCooperativeVectorHW %half %uint_64
%matrix_type = OpTypeCooperativeMatrixHW %float %uint_64 %uint_128
%result_type = OpTypeCooperativeVectorHW %float %uint_128
%_runtimearr_half = OpTypeRuntimeArray %half
%_runtimearr_float = OpTypeRuntimeArray %float
%HalfBuf = OpTypeStruct %_runtimearr_half
%FloatBuf = OpTypeStruct %_runtimearr_float
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%_ptr_StorageBuffer_FloatBuf = OpTypePointer StorageBuffer %FloatBuf
%xbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%wbuf = OpVariable %_ptr_StorageBuffer_FloatBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
%_ptr_StorageBuffer__runtimearr_float = OpTypePointer StorageBuffer %_runtimearr_float
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
%entry = OpLabel
%xbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %xbuf %int_0
%wbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %wbuf %int_0
%x = OpCooperativeVectorLoadHW %input_type %xbase %uint_0
%w = OpCooperativeMatrixLoadHW %matrix_type %wbase %shape %offset %int_0
%y = OpCooperativeVectorMatrixMulHW %result_type %x %w
OpReturn
OpFunctionEnd
)";
...
TEST_F(HwLowerToStandardTest, DirectF16RoundTripConversionsReuseOriginalLoads) {
  const std::string text = R"(
OpCapability Shader
OpCapability Float16
OpCapability CooperativeVectorHW
...
%shape = OpConstantComposite %v2uint %uint_4 %uint_4
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%vec_half = OpTypeCooperativeVectorHW %half %uint_4
%vec_float = OpTypeCooperativeVectorHW %float %uint_4
%mat_half = OpTypeCooperativeMatrixHW %half %uint_4 %uint_4
%mat_float = OpTypeCooperativeMatrixHW %float %uint_4 %uint_4
%_runtimearr_half = OpTypeRuntimeArray %half
%HalfBuf = OpTypeStruct %_runtimearr_half
%_ptr_StorageBuffer_HalfBuf = OpTypePointer StorageBuffer %HalfBuf
%xbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%wbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%bbuf = OpVariable %_ptr_StorageBuffer_HalfBuf StorageBuffer
%_ptr_StorageBuffer__runtimearr_half = OpTypePointer StorageBuffer %_runtimearr_half
%void = OpTypeVoid
%fn = OpTypeFunction %void
...
%entry = OpLabel
%xbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %xbuf %int_0
%wbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %wbuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_half %bbuf %int_0
%x = OpCooperativeVectorLoadHW %vec_half %xbase %uint_0
%x_wide = OpFConvert %vec_float %x
%x_round_trip = OpFConvert %vec_half %x_wide
%w = OpCooperativeMatrixLoadHW %mat_half %wbase %shape %offset %int_0
%w_wide = OpFConvert %mat_float %w
%w_round_trip = OpFConvert %mat_half %w_wide
%bias = OpCooperativeVectorLoadHW %vec_half %bbase %uint_0
%bias_wide = OpFConvert %vec_float %bias
%bias_round_trip = OpFConvert %vec_half %bias_wide
%y = OpCooperativeVectorMatrixMulAddHW %vec_half %x_round_trip %w_round_trip %bias_round_trip
OpReturn
OpFunctionEnd
)";
...
// ──────────────────────────────────────────────────────────────────
// P0: Shared Source Load Kill List
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest, SharedMatrixLoadPreservedAcrossTwoDirectMatmuls) {
  // B's SSBO load is shared by two matmuls.  Direct should succeed
  // for both, keeping the shared load alive.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%abuf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
%bbuf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
%c0buf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
%c1buf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
%_ptr_StorageBuffer__runtimearr_float = OpTypePointer StorageBuffer %_runtimearr_float
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
...
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%c0base = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %c0buf %int_0
%c1base = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %c1buf %int_0
%a = OpCooperativeMatrixLoadHW %mat8x8 %abase %shape %offset %int_0
%b = OpCooperativeMatrixLoadHW %mat8x8 %bbase %shape %offset %int_0
%c0 = OpCooperativeMatrixLoadHW %mat8x8 %c0base %shape %offset %int_0
%c1 = OpCooperativeMatrixLoadHW %mat8x8 %c1base %shape %offset %int_0
%d0 = OpCooperativeMatrixMulAddHW %mat8x8 %a %b %c0
%d1 = OpCooperativeMatrixMulAddHW %mat8x8 %a %b %c1
OpReturn
OpFunctionEnd
)";
  ...
  EXPECT_EQ(Pass::Status::SuccessWithChange, std::get<1>(result));
  ExpectNoHwOrCoopMatrix(disassembly);
  EXPECT_EQ(2u, CountSubstring(disassembly, "OpFunctionCall"));
  EXPECT_GT(CountSubstring(disassembly, "OpExtInst %v2float"), 0u);
}

TEST_F(HwLowerToStandardTest,
       SharedVectorLoadPreservedAcrossTwoDirectVectorMatmuls) {
  // Input vector's SSBO load is shared by two vector-matmuls with
...
%float = OpTypeFloat 32
%v2uint = OpTypeVector %uint 2
%shape0 = OpConstantComposite %v2uint %uint_16 %uint_8
%shape1 = OpConstantComposite %v2uint %uint_16 %uint_16
%offset = OpConstantComposite %v2uint %uint_0 %uint_0
%vec16 = OpTypeCooperativeVectorHW %float %uint_16
%vec8 = OpTypeCooperativeVectorHW %float %uint_8
%mat16x8 = OpTypeCooperativeMatrixHW %float %uint_16 %uint_8
...
%w1base = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %w1buf %int_0
%x = OpCooperativeVectorLoadHW %vec16 %xbase %int_0
%w0 = OpCooperativeMatrixLoadHW %mat16x8 %w0base %shape0 %offset %int_0
%w1 = OpCooperativeMatrixLoadHW %mat16x16 %w1base %shape1 %offset %int_0
%y0 = OpCooperativeVectorMatrixMulHW %vec8 %x %w0
%y1 = OpCooperativeVectorMatrixMulHW %vec16 %x %w1
OpReturn
OpFunctionEnd
)";
...
TEST_F(HwLowerToStandardTest, ExtraStoreDoesNotBlockDirectSourceSharing) {
  // Matmul followed by an extra cooperative store of B.  The extra
  // store is an HW opcode user, so it is a "safe" shared user of B's
  // load.  Direct should still succeed.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
OpEntryPoint GLCompute %main "main"
OpExecutionMode %main LocalSize 1 1 1
OpExecutionModeId %main FPFastMathDefault %float %uint_32
OpDecorate %_runtimearr_float ArrayStride 4
OpMemberDecorate %Buf 0 Offset 0
OpDecorate %Buf Block
%uint = OpTypeInt 32 0
...
OpEntryPoint GLCompute %main "main"
OpExecutionMode %main LocalSize 1 1 1
OpExecutionModeId %main FPFastMathDefault %half %uint_32
OpDecorate %_runtimearr_float ArrayStride 4
OpMemberDecorate %Buf 0 Offset 0
OpDecorate %Buf Block
%uint = OpTypeInt 32 0
...
// ──────────────────────────────────────────────────────────────────
// P1: Memory Safety Blocking
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest,
       StoreBetweenLoadAndMatmulBlocksDirectForAffectedOperand) {
  // An SSBO store to the same root as A, placed between A's load
  // and B's load.  A's direct source resolution fails (HasUnsafe
  // MemoryInstructionBetween), but B and C are unaffected.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %abuf %int_0
%a = OpCooperativeMatrixLoadHW %mat8x8 %abase %shape %offset %int_0
%store_ptr = OpAccessChain %_ptr_StorageBuffer_float %abuf %int_0 %uint_0
OpStore %store_ptr %float_1
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%b = OpCooperativeMatrixLoadHW %mat8x8 %bbase %shape %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat8x8 %cbase %shape %offset %int_0
...
TEST_F(HwLowerToStandardTest,
       ControlBarrierBetweenLoadAndMatmulBlocksDirectForAffectedOperands) {
  // A control barrier between A's load and B/C's loads.  A's direct
  // source resolution fails, but B and C are unaffected.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
TEST_F(HwLowerToStandardTest, DisjointRootStoreDoesNotBlockDirect) {
  // A store to a different SSBO root (no Aliased) between A's load
  // and the matmul should not block direct (disjoint module memory).
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%b = OpCooperativeMatrixLoadHW %mat8x8 %bbase %shape %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat8x8 %cbase %shape %offset %int_0
%store_ptr = OpAccessChain %_ptr_StorageBuffer_float %xbuf %int_0 %uint_0
OpStore %store_ptr %float_1
%d = OpCooperativeMatrixMulAddHW %mat8x8 %a %b %c
OpReturn
OpFunctionEnd
...
// ──────────────────────────────────────────────────────────────────
// P1: Value Argument (Hybrid Direct)
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest, MatrixDirectWithDynamicBPassesAsValueArgument) {
  // A and C are SSBO loads (direct source), B is a dynamic
  // OpCompositeConstruct (broadcast from a scalar loaded from SSBO)
  // → passed as a value argument to the helper.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%a = OpCooperativeMatrixLoadHW %mat4x4 %abase %shape %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat4x4 %cbase %shape %offset %int_0
%scalar_ptr = OpAccessChain %_ptr_StorageBuffer_float %dbuf %int_0 %uint_0
%scalar = OpLoad %float %scalar_ptr
%b = OpCompositeConstruct %mat4x4 %scalar
%d = OpCooperativeMatrixMulAddHW %mat4x4 %a %b %c
OpReturn
OpFunctionEnd
)";
  ...
  EXPECT_GT(CountSubstring(disassembly, "OpFunctionParameter"), 0u)
      << "Expected helper with value parameter";
}

// ──────────────────────────────────────────────────────────────────
// P2: Kill List Unclosed
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest, EscapingFunctionVariableBlocksDirect) {
  // A's load is stored to a Function variable, which is then loaded
  // and used by a non-matmul instruction (an FAdd on a scalar extract).
  // The Function variable's load escapes the kill list → direct fails.
  const std::string text = R"(
OpCapability Shader
OpCapability CooperativeMatrixHW
OpExtension "SPV_HW_neural_shader"
...
%cbuf = OpVariable %_ptr_StorageBuffer_Buf StorageBuffer
%_ptr_StorageBuffer__runtimearr_float = OpTypePointer StorageBuffer %_runtimearr_float
%_ptr_Function_mat = OpTypePointer Function %mat4x4
%void = OpTypeVoid
%fn = OpTypeFunction %void
%main = OpFunction %void None %fn
%entry = OpLabel
%avar = OpVariable %_ptr_Function_mat Function
%abase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %abuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%cbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %cbuf %int_0
%a = OpCooperativeMatrixLoadHW %mat4x4 %abase %shape %offset %int_0
OpStore %avar %a
%a_copy = OpLoad %mat4x4 %avar
%b = OpCooperativeMatrixLoadHW %mat4x4 %bbase %shape %offset %int_0
%c = OpCooperativeMatrixLoadHW %mat4x4 %cbase %shape %offset %int_0
%d = OpCooperativeMatrixMulAddHW %mat4x4 %a_copy %b %c
OpReturn
OpFunctionEnd
)";
...
// ──────────────────────────────────────────────────────────────────
// P2: Vector Function Transport
// ──────────────────────────────────────────────────────────────────

TEST_F(HwLowerToStandardTest, VectorDirectThroughFunctionVariableTransport) {
  // Vector matmul with input/matrix/bias routed through Function
  // variables (store/load pairs).  Direct should still succeed.
  const std::string text = R"(
OpCapability Shader
OpCapability FloatControls2
OpCapability CooperativeVectorHW
...
%xvar = OpVariable %_ptr_Function_vec8 Function
%wvar = OpVariable %_ptr_Function_mat8x8 Function
%bvar = OpVariable %_ptr_Function_vec8 Function
%xbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %xbuf %int_0
%wbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %wbuf %int_0
%bbase = OpAccessChain %_ptr_StorageBuffer__runtimearr_float %bbuf %int_0
%x = OpCooperativeVectorLoadHW %vec8 %xbase %int_0
...
%w = OpCooperativeMatrixLoadHW %mat8x8 %wbase %shape %offset %int_0
OpStore %wvar %w
%wv = OpLoad %mat8x8 %wvar
%bias = OpCooperativeVectorLoadHW %vec8 %bbase %int_0
OpStore %bvar %bias
%bv = OpLoad %vec8 %bvar
%d = OpCooperativeVectorMatrixMulAddHW %vec8 %xv %wv %bv
OpReturn
OpFunctionEnd
)";
